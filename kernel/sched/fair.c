...
static int best_small_task_cpu(struct task_struct *p, int sync)
{
	...
	u64 tload, cpu_load, min_busy_load = ULLONG_MAX;
	...
	for_each_cpu(i, &search_cpus) {
		 ...
		 * CPU if it's the task's previous CPU.
		 */
		if (!cpu_rq(min_cost_cpu)->cstate &&
		    mostly_idle_cpu_sync(min_cost_cpu, cpu_load, sync) &&
...
//		if (idle_cpu(i) && cstate && !sched_cpu_high_irqload(i)) {
		if (idle_cpu(i) && cstate) {
			...
			if (!spill_threshold_crossed(tload, cpu_load, rq)) {
				if (cpu_load < min_busy_load ||
				    (cpu_load == min_busy_load && i == prev_cpu)) {
					min_busy_load = cpu_load;
					best_busy_cpu = i;
				}
			}
		}
